...
|---|---|---|
| `conversion_shop` | 카카오 아이콘·제품 더 알아보기 CTA·히어로 slide-1·상세 바로구매 버튼 | **Lead** |
| `conversion_instagram` | 이벤트 메뉴·인스타 아이콘·Footer 인스타·피드 | **Contact** |
| `conversion_content` | 상품 상세 페이지 진입(경로 무관)·브랜드 페이지 진입 | **ViewContent** |
...
- Lead/Contact의 `content_name`은 **`{{dlv.source}}`**(발생 위치). 따라서:
  ...
  - Lead: `icon`·`product_more`·`hero`·`buybutton_deep`·`buybutton_bright`·`buybutton_decaf` → **상품이 content_name에 그대로 들어옴** → GTM 변경 없이 상품 구분 가능.
